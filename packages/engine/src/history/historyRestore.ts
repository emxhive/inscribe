...
function findActionEntries(repoRoot: string, actionId: string): HistoryEntry[] {
  return getHistoryEntries(repoRoot)
    .filter((entry) => isSupportedHistoryEntry(entry) && entryActionId(entry) === actionId)
...
function assertExecutionsStillCurrent(executions: PreflightExecution[]): void {
  for (const execution of executions) {
    ...
    if (current.exists !== execution.beforeExists || current.content !== execution.beforeContent) {
      throw new Error(`Workspace changed while preparing restore: ${execution.operation.file}`);
    }
  }
}
...
function resolveAction(
  ...
  if (entries.length === 0) {
    return { entries, executions: [], files: [], error: `History action not found: ${actionId}` };
  }

  const executions: PreflightExecution[] = [];
  const files = entries.map((entry, index): RestorePreviewFile => {
    ...
    if (!payload) {
      return {
        ...
        currentContent: '',
        eligible: false,
        error: 'This historical action has no restore payload and cannot be previewed safely.',
      };
    }
    ...
    try {
      ...
      const execution = resolveExactRestoreExecution(
      ...
      return {
        ...
        currentContent: currentFile.content,
        eligible: false,
        error: error instanceof Error ? error.message : 'Historical action is unsafe to restore.',
      };
    }
  });
...
export function previewRestoreAction(actionId: string, repoRoot: string): RestorePreview {
...
export function restoreAction(actionId: string, repoRoot: string): ApplyResult {
  try {
    const resolved = resolveAction(repoRoot, actionId);
    if (resolved.error || resolved.files.length === 0 || resolved.files.some((file) => !file.eligible)) {
      return { success: false, errors: [resolved.error ?? 'restore is not eligible.'] };
    }
    ...
    try {
      ...
      return { success: false, errors: [error instanceof Error ? error.message : 'restore write failed.'] };
    }
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : 'history persistence failed.';
      return {
        success: false,
        errors: [
          `history persistence failed: ${message}`,
          ...(rollbackErrors.length ? [`restore rollback failed: ${rollbackErrors.join('\n')}`] : []),
        ],
      };
    }
  } catch (error) {
    return { success: false, errors: [error instanceof Error ? error.message : 'restore failed.'] };
  }
}
