...
import { previewRestoreAction, restoreAction } from './historyRestore';
...
describe('history restore actions', () => {
  let tempDir = '';
  let repoRoot = '';
  ...
  it('previews and appends a whole-file restore action without rewriting the source action', () => {
    ...
    const preview = previewRestoreAction(actionId, repoRoot);
    expect(preview.eligible).toBe(true);
    expect(preview.files.map((file) => file.restoredState?.content)).toEqual(['old a', 'old b']);

    const restored = restoreAction(actionId, repoRoot);
    ...
    const reversalPreview = previewRestoreAction(restoreActionId!, repoRoot);
    ...
    expect(restoreAction(restoreActionId!, repoRoot).success).toBe(true);
  ...
  it('requires the exact post-action file state rather than a matching fragment', () => {
    ...
    const preview = previewRestoreAction(applied.historyEntries[0].actionId!, repoRoot);
  ...
  it('restores a created file by deleting it, then reverses that restore by recreating it', () => {
    ...
    expect(restoreAction(actionId, repoRoot).success).toBe(true);
  ...
  it('restores a deleted file by recreating it', () => {
    ...
    expect(restoreAction(applied.historyEntries[0].actionId!, repoRoot).success).toBe(true);
  ...
  it('rolls back the restore when appending inverse history fails', () => {
    ...
    try {
      const result = restoreAction(applied.historyEntries[0].actionId!, repoRoot);
  ...
  it('ignores history records without the supported history marker', () => {
    const entry = {
      id: 'legacy-action:0',
    ...
    expect(getHistoryEntries(repoRoot)).toEqual([]);
    expect(previewRestoreAction(entry.applyId, repoRoot).files).toEqual([]);
  });
});
