...
import { ApplySessionStore, sha256 } from './applySessionStore';
import type { PreviewInitialFileSnapshot } from './applySessionStore';
import type { PreviewExecutionDTO } from './previewTypes';
import type { OperationStrategy } from '@inscribe/shared';

describe('runApplyWorker', () => {
  let tempDir: string;
  let repoRoot: string;
  let store: ApplySessionStore;

  beforeEach(() => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'inscribe-apply-worker-test-'));
    repoRoot = path.join(tempDir, 'repo');
    fs.mkdirSync(repoRoot);
    store = new ApplySessionStore();
  });
  ...
  it('invalid payload returns error', async () => {
    const res = await runApplyWorker(null as any, store);
  ...
  it('rejects blank trustedRepoRoot and previewToken in payload', async () => {
    const res1 = await runApplyWorker({ trustedRepoRoot: '', previewToken: 'token' }, store);
    expect(res1.ok).toBe(false);
    if (!res1.ok) {
      expect(res1.errors[0].code).toBe('INVALID_WORKER_PAYLOAD');
    }

    const res2 = await runApplyWorker({ trustedRepoRoot: '   ', previewToken: 'token' }, store);
    expect(res2.ok).toBe(false);
    if (!res2.ok) {
      expect(res2.errors[0].code).toBe('INVALID_WORKER_PAYLOAD');
    }

    const res3 = await runApplyWorker({ trustedRepoRoot: repoRoot, previewToken: '' }, store);
    expect(res3.ok).toBe(false);
    if (!res3.ok) {
      expect(res3.errors[0].code).toBe('INVALID_WORKER_PAYLOAD');
    }

    const res4 = await runApplyWorker({ trustedRepoRoot: repoRoot, previewToken: '   ' }, store);
  ...
  it('unknown token returns error', async () => {
    const res = await runApplyWorker({ trustedRepoRoot: repoRoot, previewToken: 'unknown-token' }, store);
    expect(res.ok).toBe(false);
    if (!res.ok) {
  ...
  it('expired token returns error', async () => {
    const expiredStore = new ApplySessionStore({ sessionTtlMs: -10 });
    const sessionSummary = expiredStore.createSession(repoRoot, new Map(), []);
    const res = await runApplyWorker({ trustedRepoRoot: repoRoot, previewToken: sessionSummary.previewToken }, expiredStore);
    expect(res.ok).toBe(false);
    if (!res.ok) {
      expect(res.errors[0].type).toBe('session');
      expect(res.errors[0].code).toBe('PREVIEW_SESSION_NOT_FOUND');
  ...
  it('root mismatch returns error', async () => {
    ...
    const res = await runApplyWorker({ trustedRepoRoot: otherRoot, previewToken: sessionSummary.previewToken }, store);
    expect(res.ok).toBe(false);
    if (!res.ok) {
      expect(res.errors[0].type).toBe('session');
  ...
  it('successful create apply', async () => {
    const initial = new Map<string, PreviewInitialFileSnapshot>([
    ...
    const res = await runApplyWorker({ trustedRepoRoot: repoRoot, previewToken: sessionSummary.previewToken }, store);
  ...
  it('token cannot be reused after successful apply', async () => {
    ...
    const res1 = await runApplyWorker({ trustedRepoRoot: repoRoot, previewToken: sessionSummary.previewToken }, store);
    expect(res1.ok).toBe(true);

    const res2 = await runApplyWorker({ trustedRepoRoot: repoRoot, previewToken: sessionSummary.previewToken }, store);
    expect(res2.ok).toBe(false);
    if (!res2.ok) {
