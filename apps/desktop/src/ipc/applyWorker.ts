...
import { ApplySessionStore, defaultApplySessionStore } from './applySessionStore';
import type {
  ApplyWorkerPayload,
  ApplyWorkerResponse,
  ApplyErrorDTO,
...
export async function runApplyWorker(
  ...
  try {
    ...
    if (
      ...
      return {
        ...
        errors: [{
          ...
          message: 'Invalid apply worker payload.',
        }],
      };
    }
    ...
    if (!result.ok) {
      const errors: ApplyErrorDTO[] = result.errors.map((err) => {
        let type: ApplyErrorType = 'system';
        if (
        ...
        let publicMessage = 'An unexpected error occurred during apply.';
        switch (err.code) {
          ...
          case 'APPLY_WRITE_FAILED':
            publicMessage = 'apply failed while writing files.';
            break;
          case 'HISTORY_PERSISTENCE_FAILED':
            publicMessage = 'apply failed while recording history.';
            break;
          case 'ROLLBACK_FAILED':
            publicMessage = 'apply rollback did not complete.';
            break;
        }
