...
export interface PreviewInitialFileSnapshot {
  exists: boolean;
  content: string;
  hash: string;
}
...
export interface PreviewSession {
  ...
  createdAt: string;
  expiresAt: string;
  initialFiles: Map<string, PreviewInitialFileSnapshot>;
...
export function cloneSnapshotMap(
  map: Map<string, PreviewInitialFileSnapshot>
): Map<string, PreviewInitialFileSnapshot> {
  const cloned = new Map<string, PreviewInitialFileSnapshot>();
...
export function cloneExecutions(execs: PreviewExecutionDTO[]): PreviewExecutionDTO[] {
  return execs.map((e) => ({
...
export function collapseExecutions(
  ...
  const fileToExecs = new Map<string, PreviewExecutionDTO[]>();
  for (const exec of executions) {
...
export class ApplySessionStore {
  ...
  public consumeSession(token: string, repoRoot: string): PreviewSession {
    this.cleanupExpired();
...
export const defaultApplySessionStore = new ApplySessionStore();
