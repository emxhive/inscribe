...
- `capability`: a developer intent that must remain visible in the benchmark even when the current vocabulary cannot express it. These cases describe the desired target and the evidence a future structural vocabulary should provide; they are not forced into an inaccurate selector. Ordinary capability cases remain TODOs. Cases marked `parser-compatibility` instead execute the registered V2 syntax-validation path and assert their declared parser outcome, so grammar support changes become visible benchmark results rather than silent TODOs.
