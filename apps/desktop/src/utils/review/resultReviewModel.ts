...
export function buildResultReviewModel(
  comparison: ReviewComparison,
): ReviewRenderModel {
  const hunks =
    (comparison.regions?.length ?? 0) > 0
      ? comparison.regions!
  ...
  return {
    ...
    regions: hunks.map((region) => ({
      ...
      anchorOffset:
        'renderAnchor' in region
          ? region.renderAnchor.newOffset
          : region.newRange.start,
      anchorSide:
        'renderAnchor' in region
          ? region.renderAnchor.side
          : 'before',
      deletedSummary:
        region.kind === 'delete'
          ? summarizeDeletedText(region.oldText)
          : null,
    })),
    windows: (
      comparison.replacementRegions ??
      comparison.regions
    ).map((region) => ({
      id: region.id,
...
export const buildReviewRenderModel =
  buildResultReviewModel;

export function buildReviewRegionOverlay(
  region:
    | OperationComparisonRegion
    | OperationDiffHunk,
): ReviewRegionOverlayModel {
  return {
...
export function summarizeDeletedText(
  text: string,
): string {
