...
export function scanIntakeStructure(
  ...
  const finalizeBlock = (active: ActiveBlock, endLineIndex: number, unterminated = false) => {
    ...
    if (!fileDirective) {
      ...
      if (!fileDirective.value) {
        ...
        const pathErr = validateRelativeFilePath(fileDirective.value);
        if (pathErr) {
  ...
  rawLines.forEach((lineText, lineIndex) => {
    ...
    if (trimmed === INSCRIBE_BLOCK_CLOSE) {
      if (currentBlock) {
    ...
    if (currentBlock.activeSection) {
      if (isExactMarkerLine(lineText)) {
        ...
        if (trimmed === expectedCloser) {
          ...
          return;
        } else if (trimmed === INSCRIBE_BLOCK_OPEN) {
          ...
          linesMeta[lineIndex].blockId = currentBlock.id;
          return;
        } else if (trimmed === INSCRIBE_BLOCK_CLOSE) {
          ...
          const secName = trimmed.slice(3) as SectionName;
    ...
    const isSectionOpener = trimmed.startsWith('<<<') && trimmed !== INSCRIBE_BLOCK_OPEN;
    const isSectionCloser = trimmed.endsWith('>>>') && trimmed !== INSCRIBE_BLOCK_CLOSE;

    if (isSectionOpener) {
      const secName = trimmed.slice(3) as SectionName;
      const isValidSec = SECTION_NAMES.includes(secName);
    ...
    if (isSectionCloser) {
      const secName = trimmed.slice(0, -3) as SectionName;
      const isValidSec = SECTION_NAMES.includes(secName);
    ...
    if (colonIdx !== -1) {
      ...
      const isValidDir = DIRECTIVE_KEYS.includes(key as DirectiveKey);
      if (!isValidDir) {
