...
const mockPreviewOnWorker = vi.hoisted(() => vi.fn());
...
vi.mock('./engineWorkerClient', () => ({
  previewOnWorker: mockPreviewOnWorker,
}));
...
describe('preview IPC Route Validation', () => {
  beforeEach(() => {
    vi.restoreAllMocks();
  });

  it('registers the preview IPC handler', () => {
    registerPreviewHandlers();
    expect(ipcMain.handle).toHaveBeenCalledWith('preview', expect.any(Function));
  });
  ...
  it('uses requireTrustedRepoRoot and ignores renderer-supplied assetPaths', async () => {
    ...
    mockPreviewOnWorker.mockResolvedValue({
      ok: true,
      partial: false,
      executions: [],
      errors: [],
    ...
    // previewOnWorker should receive trusted root and internally derived paths, ignoring args.assetPaths
    expect(mockPreviewOnWorker).toHaveBeenCalledWith(
