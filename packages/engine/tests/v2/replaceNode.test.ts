...
describe("replace_node operation", () => {
  it("replaces a named TypeScript class method", async () => {
    ...
    const virtualState = new Map([
      ["test.ts", { content: source, exists: true }],
    ]);
    const op = {
    ...
    expect(exec.afterContent).toBe(
      `class Greeter {\n  greet() {\n    return "hi";\n  }\n}`,
    );
    expect(exec.targetScope.beforeRange).toBeDefined();

    const slice = source.slice(
      exec.targetScope.beforeRange!.start,
      exec.targetScope.beforeRange!.end,
    );
...
function tail() {}`);
  ...
  it("replaces an exported named TSX function component", async () => {
    ...
    const virtualState = new Map([
      ["test.tsx", { content: source, exists: true }],
    ]);
    const op = {
      strategy: "replace_node" as const,
      filePath: "test.tsx",
  ...
  it("fails with TARGET_AMBIGUOUS for ambiguous structural selector", async () => {
    ...
    await expect(resolveOperation(op, virtualState, CONTEXT)).rejects.toThrow(
      "TARGET_AMBIGUOUS",
    );
  });
  ...
  it("fails with UNSUPPORTED_EXTENSION for unsupported extensions", async () => {
    ...
    const virtualState = new Map([
      ["test.java", { content: source, exists: true }],
    ]);
    const op = {
      strategy: "replace_node" as const,
      filePath: "test.java",
  ...
  it("fails with STRUCTURAL_RESOLVER_REQUIRED when structural resolver is missing", async () => {
    ...
    await expect(resolveOperation(op, virtualState, {})).rejects.toThrow(
  ...
  it("returns not-found when malformed source does not expose the requested target", async () => {
    const source = `function Comp() {\n  if (true) {\n    return 42;\n  \n}`;
    const virtualState = new Map([
      ["test.ts", { content: source, exists: true }],
    ]);
  ...
  it("verifies parser and tree disposal", async () => {
    ...
    const createParserSpy = vi
      .spyOn(runtime, "createParser")
      .mockImplementation(() => {
          ...
          get(target, prop, receiver) {
            ...
            if (prop === "parse") {
              return function (this: any, ...args: any[]) {
                const realTree = realParser.parse.apply(
                  realParser,
                  args as any,
                );
    ...
    try {
      ...
      parserDeleteSpies = [];
      treeDeleteSpies = [];
      await runResolution("function Comp() {}", [
        { kind: "function", name: "Comp" },
      ]);
      ...
      treeDeleteSpies = [];
      await expect(
        runResolution("function Comp() {}", [{ kind: "class", name: "Comp" }]),
      ...
      const virtualStateAmb = new Map([
        ["test.ts", { content: sourceAmb, exists: true }],
      ]);
      const opAmb = {
        strategy: "replace_node" as const,
        filePath: "test.ts",
        content: "  if (c) {}",
        selector: {
          path: [{ kind: "function", name: "Comp" }, { kind: "if_statement" }],
        },
      };
      await expect(
        resolveOperation(opAmb, virtualStateAmb, CONTEXT),
      ).rejects.toThrow("TARGET_AMBIGUOUS");
      expect(parserDeleteSpies[0]).toHaveBeenCalled();
      expect(treeDeleteSpies[0]).toHaveBeenCalled();

      // 4. Parser disposal also occurs for structurally unresolved targets.
      parserDeleteSpies = [];
      treeDeleteSpies = [];
      const sourceDiag = "function (";
      const virtualStateDiag = new Map([
        ["test.ts", { content: sourceDiag, exists: true }],
      ]);
      const opDiag = {
        strategy: "replace_node" as const,
        filePath: "test.ts",
      ...
      await expect(
        resolveOperation(opDiag, virtualStateDiag, CONTEXT),
      ).rejects.toThrow("TARGET_NOT_FOUND");
      expect(parserDeleteSpies[0]).toHaveBeenCalled();
      expect(treeDeleteSpies[0]).toHaveBeenCalled();
    } finally {
  ...
  it("enforces scope-aware traversal boundaries", async () => {
    ...
    const virtualState1 = new Map([
      ["test.ts", { content: source1, exists: true }],
    ]);
    const op1 = {
      strategy: "replace_node" as const,
      filePath: "test.ts",
    ...
    await expect(resolveOperation(op1, virtualState1, CONTEXT)).rejects.toThrow(
      "TARGET_NOT_FOUND",
    );
    ...
    await expect(
      resolveOperation(op2, virtualState1, CONTEXT),
    ).resolves.toBeDefined();

    // 2. Class expression inside function
    const source2 = `function outer() {\n  const Inner = class {\n    save() {}\n  };\n}`;
    const virtualState2 = new Map([
      ["test.ts", { content: source2, exists: true }],
    ]);
    const op3 = {
      strategy: "replace_node" as const,
      filePath: "test.ts",
    ...
    await expect(resolveOperation(op3, virtualState2, CONTEXT)).rejects.toThrow(
      "TARGET_NOT_FOUND",
    );

    // 3. Arrow function expression scope barrier
    const source3 = `function outer() {\n  items.forEach(() => {\n    if (hidden) {\n      run();\n    }\n  });\n}`;
    const virtualState3 = new Map([
      ["test.ts", { content: source3, exists: true }],
    ]);
    const op4 = {
      strategy: "replace_node" as const,
      filePath: "test.ts",
    ...
    await expect(resolveOperation(op4, virtualState3, CONTEXT)).rejects.toThrow(
      "TARGET_NOT_FOUND",
    );

    // 4. Positive test: direct if-statement resolves
    const source4 = `function outer() {\n  if (visible) {\n    run();\n  }\n}`;
    const virtualState4 = new Map([
      ["test.ts", { content: source4, exists: true }],
    ]);
    const op5 = {
      strategy: "replace_node" as const,
      filePath: "test.ts",
  ...
  it("asserts exact logical replacement ranges including wrappers", async () => {
    ...
    expect(
      await getRangeSlice(s1, [{ kind: "function", name: "UserCard" }]),
    ).toBe(s1);

    // 2. Default-exported function
    const s2 = `export default function UserCard() {\n  return <div />;\n}`;
    expect(
      await getRangeSlice(s2, [{ kind: "function", name: "UserCard" }]),
    ).toBe(s2);

    // 3. Exported arrow component
    const s3 = `export const UserCard = () => {\n  return <div />;\n};`;
    expect(
      await getRangeSlice(s3, [{ kind: "function", name: "UserCard" }]),
    ).toBe(s3);

    // 4. Non-exported arrow function
    const s4 = `const UserCard = () => {\n  return <div />;\n};`;
    expect(
      await getRangeSlice(s4, [{ kind: "function", name: "UserCard" }]),
    ).toBe(s4);

    // 5. Exported class
    const s5 = `export class UserService {}`;
    expect(
      await getRangeSlice(s5, [{ kind: "class", name: "UserService" }]),
    ).toBe(s5);

    // 6. Non-exported class
    const s6 = `class UserService {}`;
    expect(
      await getRangeSlice(s6, [{ kind: "class", name: "UserService" }]),
    ).toBe(s6);

    // 7. var arrow function
    const s7 = `var UserCard = () => {\n  return <div />;\n};`;
    expect(
      await getRangeSlice(s7, [{ kind: "function", name: "UserCard" }]),
    ).toBe(s7);

    // 8. var function expression
    const s8 = `var UserCard = function () {\n  return <div />;\n};`;
    expect(
      await getRangeSlice(s8, [{ kind: "function", name: "UserCard" }]),
    ).toBe(s8);
  });
  ...
  it("asserts INVALID_SELECTOR for resolver-boundary violations including blank metadata", async () => {
    ...
    await expect(resolveOperation(op1, virtualState, CONTEXT)).rejects.toThrow(
      "INVALID_SELECTOR",
    );
    ...
    await expect(resolveOperation(op2, virtualState, CONTEXT)).rejects.toThrow(
      "INVALID_SELECTOR",
    );
    ...
    await expect(resolveOperation(op3, virtualState, CONTEXT)).rejects.toThrow(
      "INVALID_SELECTOR",
    );
    ...
    await expect(resolveOperation(op4, virtualState, CONTEXT)).rejects.toThrow(
      "INVALID_SELECTOR",
    );
    ...
    const op5: any = {
      ...
      filePath: "test.ts",
      content: "test",
      selector: { path: [{ kind: "unsupported" }] },
    };
    await expect(resolveOperation(op5, virtualState, CONTEXT)).rejects.toThrow(
      "INVALID_SELECTOR",
    );
    ...
    const op6: any = {
      ...
      filePath: "test.ts",
      content: "test",
      selector: { path: [{ kind: "function", name: 123 }] },
    };
    await expect(resolveOperation(op6, virtualState, CONTEXT)).rejects.toThrow(
      "INVALID_SELECTOR",
    );
    ...
    const op7: any = {
      ...
      filePath: "test.ts",
      content: "test",
      selector: { path: [{ kind: "function", name: "" }] },
    };
    await expect(resolveOperation(op7, virtualState, CONTEXT)).rejects.toThrow(
      "INVALID_SELECTOR",
    );
    ...
    await expect(resolveOperation(op8, virtualState, CONTEXT)).rejects.toThrow(
      "INVALID_SELECTOR",
    );
    ...
    const op9: any = {
      ...
      content: "test",
      selector: {
        path: [{ kind: "function", name: "outer" }],
        startsWith: 123,
      },
    };
    await expect(resolveOperation(op9, virtualState, CONTEXT)).rejects.toThrow(
      "INVALID_SELECTOR",
    );
    ...
    const op10: any = {
      ...
      filePath: "test.ts",
      content: "test",
      selector: { path: [{ kind: "function", name: "outer" }], startsWith: "" },
    };
    await expect(resolveOperation(op10, virtualState, CONTEXT)).rejects.toThrow(
      "INVALID_SELECTOR",
    );
    ...
    await expect(resolveOperation(op11, virtualState, CONTEXT)).rejects.toThrow(
      "INVALID_SELECTOR",
    );
  });
});
