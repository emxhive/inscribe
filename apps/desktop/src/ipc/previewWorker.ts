...
import { loadInitialFilesRecovering } from './previewWorkspace';
import type {
  PreviewWorkerPayload,
  PreviewWorkerResponse,
  PreviewExecutionDTO,
  PreviewErrorDTO,
} from './previewTypes';
import { ApplySessionStore, PreviewInitialFileSnapshot, defaultApplySessionStore } from './applySessionStore';
...
export async function runPreviewWorker(
  ...
  sessionStore: ApplySessionStore = defaultApplySessionStore
): Promise<PreviewWorkerResponse> {
      ...
      errors: [
        {
          ...
          message: 'Invalid preview worker payload.',
        },
      ],
    };
  }
  ...
  try {
    const parsed = parseInscribeBlocksRecovering(rawInput);
    const errors: PreviewErrorDTO[] = parsed.diagnostics.map((diagnostic) => ({
      type: 'protocol',
    ...
    if (operations.length === 0) {
      return {
        ...
        errors: errors.length > 0 ? errors : [{
          ...
          message: 'No operations could be previewed.',
        }],
      };
    }
    ...
    const resolvedPlan = await resolvePlan(operations, initialFiles, {
      structuralResolver,
      syntaxValidator,
    });
    ...
    const executions: PreviewExecutionDTO[] = resolvedPlan.executions.flatMap(
      (exec, idx) => {
    ...
    let sessionSummary;
    try {
      const snapshotMap = new Map<string, PreviewInitialFileSnapshot>();
    ...
    return {
      ...
      executions,
      finalFiles: sessionSummary.finalMutations.map((mutation) => ({
        ...
        beforeFileHash: mutation.beforeFileHash,
        afterFileHash: mutation.afterFileHash,
        actualDiffHunks: computeDiffHunks(mutation.beforeContent, mutation.afterContent),
      })),
      errors,
